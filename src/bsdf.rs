...
pub trait Bsdf: Sync + Send + Base {
    ...
    fn evaluate_pdf(&self, sp: &ShadingPoint, wo: Vec3, wi: Vec3) -> f32;
...
impl<'a> BsdfClosure<'a> {
    pub fn evaluate(&self, wo: Vec3, wi: Vec3) -> Spectrum {
        self.bsdf
            .evaluate(&self.sp, self.frame.to_local(wo), self.frame.to_local(wi))
    }
    pub fn evaluate_pdf(&self, wo: Vec3, wi: Vec3) -> f32 {
        self.bsdf
            .evaluate_pdf(&self.sp, self.frame.to_local(wo), self.frame.to_local(wi))
    }
...
impl Bsdf for NullBsdf {
    ...
    fn evaluate_pdf(&self, _sp: &ShadingPoint, _wo: Vec3, _wi: Vec3) -> f32 {
        0.0
    }
...
impl Bsdf for DiffuseBsdf {
    ...
    fn evaluate_pdf(&self, _sp: &ShadingPoint, wo: Vec3, wi: Vec3) -> f32 {
        if Frame::same_hemisphere(wo, wi) {
