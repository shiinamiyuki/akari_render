        ...
        T: BVHData + Sync + 'static,
    {
        pub fn build(data: T, mut references: Vec<u32>) -> BVHAccelerator<T> {
            ...
            let (data, nodes, aabb) = {
                ...
                builder.recursive_build(0, references.len() as u32, 0, &mut references);
                // bvh.references = references;
            ...
            if end - begin <= 4 || depth >= 30 {
                ...
                macro_rules! try_split_with_axis {
                    ($axis:expr) => {{
                        ...
                        let mut split = 0;
                        for i in begin..end {
                            let cost = (i - begin) as f64 * surface_area_fwd[(i - begin) as usize]
                                + (end - i - 1) as f64 * surface_area_rev[(i - begin) as usize];
                            if cost < min_cost {
                                split = i;
                                min_cost = cost;
                            }
                ...
                // partition
                {
                    ...
                    let (left, right) = {
                        ...
                        if end - begin >= 128 * 1024
                            && depth <= (num_threads as f64).log2().ceil() as u32
                        {
                            rayon::join(
                                || {
                                    self.recursive_build(begin, mid, depth + 1, unsafe {
                                        p_ref.as_mut().unwrap()
                                    })
                                },
                                || {
                                    self.recursive_build(mid, end, depth + 1, unsafe {
                                        p_ref.as_mut().unwrap()
                                    })
                                },
                            )
                        } else {
                            let left = self.recursive_build(begin, mid, depth + 1, references);

                            let right = self.recursive_build(mid, end, depth + 1, references);
                            (left, right)
                        }
                    };
