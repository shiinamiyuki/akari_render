...
namespace akari::engine {
    void AppWindow::setup_vulkan(const char **extensions, uint32_t extensions_count) {
        ...
        // Create Vulkan Instance
        {
            ...
            CHECK_VK(vkCreateInstance(&create_info, g_Allocator, &g_Instance));
        }

        // Select GPU
        {
            uint32_t gpu_count;
            CHECK_VK(vkEnumeratePhysicalDevices(g_Instance, &gpu_count, nullptr));
            IM_ASSERT(gpu_count > 0);

            VkPhysicalDevice *gpus = new VkPhysicalDevice[gpu_count];
            CHECK_VK(vkEnumeratePhysicalDevices(g_Instance, &gpu_count, gpus));
        ...
        // Select graphics queue family
        {
            ...
            delete[] queues;
            AKR_ASSERT(g_QueueFamily != (uint32_t)-1);
        }
        ...
        // Create Logical Device (with 1 queue)
        {
            ...
            CHECK_VK(vkCreateDevice(g_PhysicalDevice, &create_info, g_Allocator, &g_Device));
        ...
        // Create Descriptor Pool
        {
            ...
            CHECK_VK(vkCreateDescriptorPool(g_Device, &pool_info, g_Allocator, &g_DescriptorPool));
        }
    }
...
#    endif
    ...
    void AppWindow::show() {
        ...
        // Upload Fonts
        {
            ...
            CHECK_VK(vkResetCommandPool(g_Device, command_pool, 0));
            ...
            CHECK_VK(vkQueueSubmit(g_Queue, 1, &end_info, VK_NULL_HANDLE));
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            if (!is_minimized) {
                std::memcpy(&wd->ClearValue.color.float32[0], &clear_color, 4 * sizeof(float));
    ...
    void AppWindow::frame_render(ImDrawData *draw_data) {
        auto *wd = &g_MainWindowData;
        VkResult err;
