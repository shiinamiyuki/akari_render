...
#ifdef NDEBUG
#define RT_DEBUG_ASSERT(expr)
#else
#define RT_DEBUG_ASSERT(expr) RT_ASSERT(expr)
#endif
...
struct EmbreeTlasImpl {
  ...
  bool traverse(EmbreeRayQuery &rq) {
    RTCRay ray{};
    ...
    struct RqContext : RTCRayQueryContext {
      EmbreeRayQuery &rq;
      RqContext(EmbreeRayQuery &rq) : rq(rq) {}
    };
